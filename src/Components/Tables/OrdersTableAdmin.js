...
import {CREATED, PROCESSING, CANCELED, COMPLETED } from "../../Constants/parameters";
        ...
        <tbody className="table-group-divider bg-light">
          { currentOrders?.map((order, index) => (
            <tr key={order.id} className="align-middle">
              ...
              <td> { (order.state === "created") ?
                                    <span className="bg-secondary text-white border border-primary rounded-pill item_state">{order.state.toString().toUpperCase()}</span>
                          : (order.state === "canceled") ?            
                                    <span className="bg-danger text-white border border-primary rounded-pill item_state">{order.state.toString().toUpperCase()}</span>
                          : (order.state === "processing") ?
                                    <span className="bg-warning text-white border border-primary rounded-pill item_state">{order.state.toString().toUpperCase()}</span>
                          : (order.state === "completed") ?
                                    <span className="border border-primary rounded-pill item_state item_state_completed">{order.state.toString().toUpperCase()}</span>
                           : ""}
              </td>
              ...
              <td className="justify-content-center gap-3">
                <button
                    ...
                    disabled={(order.state === "canceled") || (order.state === "completed")}
